...
class FrequencyRepresentationModule(nn.Module):
    def __init__(self, signal_dim=50, n_filters=8, n_layers=3, inner_dim=125,
        ...
        self.encoder = nn.Sequential( # like the Composition layer you built
            nn.Conv1d(1,ae_filters,kernel_size,self.stride,self.padding_conv),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.LeakyReLU(0.2),
            # nn.SELU(),
            nn.ConvTranspose1d(ae_filters, 1, kernel_size, self.stride, self.padding_trans, self.output_padding),
            nn.BatchNorm1d(1),
            nn.LeakyReLU(0.2),
            # nn.SELU(),
            )
    ...
    def forward(self, inp):  
        ...
        x = x1.view(bsz,-1)       
        x = self.in_layer(x) #.view(bsz, self.n_filters, -1)
